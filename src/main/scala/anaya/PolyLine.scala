...
class PolyLine(
  ...
  private [anaya] def contains(point:Point): Boolean = {
    var startIndex = 0
    var endIndex = 1
    var contains = false
  ...
  /*override def jsonValue: JValue =
    ("type" -> "udt") ~
      ...
      ("pyClass" -> "anaya.types.PolyLineUDT") ~
      ("indices" -> JArray(indices.map(index => JInt(index)).toList)) ~
      ("points" -> JArray(points.map(_.jsonValue).toList))*/
}

private[anaya] class PolyLineUDT extends UserDefinedType[PolyLine] {
  ...
  override def pyUDT: String = "anaya.types.PolyLineUDT"
}

private[anaya] object PolyLine {
